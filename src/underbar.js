(function() {
  ...
  _.identity = function(val) {
    return val; //simply return what was passed in
  };
  ...
  _.last = function(array, n) {
    //We handle the edge case where n > length of the array
    if (n > array.length) { return array; }
    //Then, we slice the array or return the last element if n is undefined
  ...
  _.each = function(collection, iterator) {
    //Requires seperate logic for arrays and objects (?)
    //First, we handle the array case:
    if (Array.isArray(collection)){
      //A for loop gives us easy access to the numeric key, so we use one here
      for (var i = 0; i<collection.length; i++) {
        iterator(collection[i], i, collection);
      }
    //Then, we handle object case:
  } else { //We know collection is an object if it's not an array
      //A for-in loop gives us access to object keys, so we use one here
  ...
  _.reduce = function(collection, iterator, accumulator) {
    //We use each to iterate through collection. If we're on the first step and
    //accumulator has not been passed in, we set it to the first value and continue.
    //Otherwise, we call iterator and store the result as accumulator.
    _.each(collection, function(item, index){
      accumulator = (accumulator === undefined && index === 0) ? _.first(collection) : iterator(accumulator, item);
    });
    return accumulator;
  };
